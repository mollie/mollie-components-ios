...
// for network resilience (epic 315, Plan B): a payment sheet must not inherit
// URLSession.shared's untuned 60s request / 7-day resource defaults.
import MollieComponents

final class MolliePaymentEndpointsTests: XCTestCase {
    ...
    func test_production_requestTimeout_isTuned() {
        let config = MolliePaymentEndpoints.production.urlSession.configuration
        XCTAssertEqual(
