...
| --- | --- | --- |
| Secure text entry: **on** for CVC, **off** for PAN (by design) | `isSecureTextEntry = true` on the CVC field — it is the only iOS control that forces the system keyboard, which locks third-party keyboard extensions out of that field even when the user has granted one Full Access. `isSecureTextEntry` stays `false` on the PAN, deliberately: card-entry UX requires the user to see the digits they type, and masking drives typos and retries that re-expose the PAN more than a screen recorder would. **Masking and keyboard isolation are the same switch on iOS — there is no API to get one without the other** — which is why the tradeoff is taken on the short field (3-4 digits, cheap to mask) and not the long one. **Two residual points.** (1) *Screen capture and observation*, on both fields: covered by the app-switcher and screen-recording privacy overlays, pasteboard and edit-menu (copy / cut / share) blocking, masked accessibility values, and the best-effort field wipe — see the rows below. (2) *Keystroke capture on the PAN: not covered.* Leaving it `false` means a third-party keyboard extension granted Full Access can still observe the PAN as it is typed; the CVC's keystrokes are now closed to that channel. The SDK cannot block keyboard extensions on the merchant's behalf — `application(_:shouldAllowExtensionPointIdentifier:)` is a `UIApplicationDelegate` method only the host app can implement; merchants handling card data should return `false` for `.keyboard` to close the PAN's remaining gap. | Locks third-party keyboard extensions out of the CVC keystroke channel; the PAN's keystroke channel remains an accepted residual with a merchant-side control. Screen-capture defenses for both fields are listed in the following rows. |
