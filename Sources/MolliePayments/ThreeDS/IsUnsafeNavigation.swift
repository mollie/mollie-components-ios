...
///
/// Lives outside `ThreeDSWebViewController` (and outside the UIKit/WebKit
/// `#if`) so it can be unit-tested on macOS where `UIKit` is unavailable.
...
private func isUnsafeIPv4(_ addr: IPv4Address) -> Bool {
    ...
    if first == 192, second == 0, bytes[2] == 0 { return true }
    ...
    if first == 168, second == 63, bytes[2] == 129, bytes[3] == 16 { return true }
    return false
}
...
private func isUnsafeIPv6(_ addr: IPv6Address) -> Bool {
       ...
       bytes.dropFirst(4).prefix(8).allSatisfy({ $0 == 0 })
    {
        ...
        if let mapped { return isUnsafeIPv4(mapped) }
        return true
    }
