...
/// HTTP client for the PCI Tokeniser service. Unauthenticated by design — the
/// tokeniser endpoints (`POST /v1/tokens`, `GET /v1/iin/{prefix}`) accept no
/// bearer credentials. Lives on a separate host from the Sessions Service
/// (e.g. `https://tokenize.mollie.com`) so it MUST be constructed with its
/// own `baseURL`, never shared with `SessionClient`.
public final class TokenizerClient: HTTPClient, @unchecked Sendable {
    ...
    public func perform<T: Decodable>(_ endpoint: Endpoint<T>) async throws -> T {
        let request = try endpoint.asURLRequest(baseURL: baseURL, clientAccessToken: nil)
    ...
    private func execute<T: Decodable>(
        ...
        MollieLogger.log("TokenizerClient", "→ \(request.httpMethod ?? "?") \(request.url?.absoluteString ?? "?")")
        // Do not log request body — contains PAN + CVC (PCI-sensitive).
        do {
            let (data, response) = try await session.data(for: request)
            let httpResponse = response as! HTTPURLResponse // swiftlint:disable:this force_cast
            MollieLogger.log("TokenizerClient", "← HTTP \(httpResponse.statusCode)")
            // Do not log response body — contains card token / echoed cardholder data.
            ...
            // idempotent ops only, honouring any Retry-After. The tokenise POST
            // is excluded via isIdempotent (spike #316, Model B).
